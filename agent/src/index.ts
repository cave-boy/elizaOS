...
import { mainCharacter } from "./mainCharacter.ts"; // Ensure this exists
import { kaleemCharacter } from "./kaleemCharacter.ts"; // Ensure this exists
import { loadEnabledSanityCharacters } from "@elizaos-plugins/plugin-sanity"; // Correct import from package
...
export function parseArguments(): {
    ...
    try {
        const argv = process.argv.slice(2).filter(arg => arg !== "--isRoot");
        elizaLogger.info("Raw ARGV for parsing:", argv);
        return yargs(argv)
            .option("character", {
                type: "string",
                description: "Path to the character JSON file (optional, overrides Sanity if provided)",
            })
            .option("characters", {
                type: "string",
                description: "Comma separated list of paths to character JSON files (optional, overrides Sanity if provided)",
            })
            .parseSync();
    } catch (error) {
...
const hasValidRemoteUrls = () =>
    ...
    const startAgents = async () => {
        ...
        // If CLI arguments are provided, load from files; otherwise, fetch from Sanity
        if (charactersArg) {
            elizaLogger.info("Loading characters from CLI arguments");
            characters = await loadCharacters(charactersArg); // Assumes loadCharacters exists
        } else {
            elizaLogger.info("Fetching enabled characters from Sanity");
            characters = await loadEnabledSanityCharacters();
            if (characters.length === 0) {
                elizaLogger.warn("No enabled characters found in Sanity, no agents will start");
            }
        }
    
        elizaLogger.info("Characters to start:", characters.map(c => c.name));
        ...
        directClient.startAgent = async (character) => {
            character.plugins = await handlePluginImporting(character.plugins); // Assumes handlePluginImporting exists
            return startAgent(character, directClient);
        };
    
        directClient.loadCharacterTryPath = loadCharacterTryPath; // Assumes this exists
        directClient.jsonToCharacter = jsonToCharacter; // Assumes this exists
